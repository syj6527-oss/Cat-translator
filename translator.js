...
export function assemblePrompt(text, targetLang, isToEnglish, settings, options = {}) {
    ...
    const bilingualMode = settings.dialogueBilingual || 'off';
    const narrationPrompt = String(settings.narrationPrompt || settings.userPrompt || '').trim();
    const dialoguePrompt = String(settings.dialoguePrompt || '').trim();
    
    // 🚨 병기 모드 ON이면 짧은 텍스트도 풀 프롬프트 경로 강제 사용
    if (bilingualMode === 'off' && settings.literalBilingual !== 'on' && !structureProtected && !retryReason && text.length < 100 && !prevTranslation && contextMessages.length === 0 && (!settings.dictionary || !settings.dictionary.trim()) && !narrationPrompt && !dialoguePrompt) {
