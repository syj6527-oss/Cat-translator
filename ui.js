...
export function setupSettingsPanel(settings, stContext, saveSettingsFn) {
    ...
    // 🚨 DOM 구조 대공사: API Key 발바닥 위치 & 사전 우편함 정렬 수정!
    const html = `
    <div id="cat-trans-container" class="inline-drawer">
        <div id="cat-drawer-header" class="inline-drawer-header interactable" tabindex="0">
            <div class="inline-drawer-title"><span class="cat-theme-emoji">🐱</span><span>트랜스레이터 Beta v18.3.0</span></div>
        ...
        <div id="cat-drawer-content" class="inline-drawer-content" style="display:none; padding:10px;">
            ...
            <div id="ct-direct-settings" style="display:${settings.profile === '' ? 'block' : 'none'};">
                <div class="cat-setting-row">
                    ...
                    <div style="position:relative;">
                        <input type="password" id="ct-key" class="text_pole" value="${settings.customKey}" style="padding-right:36px; width:100%; box-sizing:border-box;">
            ...
            <div class="cat-setting-row">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:3px;">
                    <label style="margin-bottom:0;">사전 (원문 = 번역어)</label>
                    <span id="ct-dict-reset" style="cursor:pointer; font-size:1.1em; transition:0.2s;" title="사전 지우기 (우편함 비우기)">${dictIcon}</span>
                </div>
    ...
    // 🚨 감지 로직 딜레이: 드롭다운 메뉴가 닫힌 직후에 토스트를 띄우도록 설정!
    ...
    $('#ct-dictionary').on('input', function () { settings.dictionary = $(this).val(); $('#ct-dict-reset').text(settings.dictionary.trim() ? '📬' : '📭'); });
    $('#ct-dict-reset').on('click', async function() { $('#ct-dictionary').val(''); settings.dictionary = ''; saveSettingsFn(); $(this).text('📭'); await clearAllCache(); updateCacheStats(); catNotify(`${getThemeEmoji()} 📭 우편함 비우기 + 캐시 초기화 완료!`, "success"); });
...
export function injectMessageButtons(processMessageFn, revertMessageFn) {
    $('.mes:not(:has(.cat-btn-group))').each(function () {
        ...
        if (target.length > 0) { 
            ...
            let sysWrap = $('<div class="cat-sys-wrap" style="text-align:right; margin-bottom:4px; margin-right:8px; display:flex; justify-content:flex-end;"></div>'); 
...
function showBulkPopup(event, settings, stContext, processMessageFn) {
    ...
    if (btn) {
        ...
        popup.css({ position: 'fixed', bottom: (window.innerHeight - rect.top + 8) + 'px', left: (rect.left - 40) + 'px', zIndex: 2147483647 });
    }
    $('body').append(popup);
...
async function executeBulkTranslation(count, settings, stContext, processMessageFn) {
    ...
    // 🚨 기획서 반영: 번역 시작 시 번개(⚡) 아이콘을 빨간색 취소(X) 아이콘으로 변경!
    $('#cat-bulk-btn').html('<i class="fa-solid fa-xmark" style="color: #e74c3c; font-size: 1.4em;"></i>');
    
    const abortHandler = () => { if (bulkAbortController) bulkAbortController.abort(); };
    $('#cat-bulk-btn').off('click').on('click', (e) => { e.preventDefault(); e.stopPropagation(); abortHandler(); });

    // 기획서 반영: catNotify로 진행률 표시
    const progressEl = catNotifyProgress(`${getThemeEmoji()} 전체 번역 중... (0/${total})`, abortHandler);
    ...
    for (const el of targets) {
        ...
        if (progressEl.length) progressEl.text(`${getThemeEmoji()} 전체 번역 중... (${completed}/${total})`);
        
        // 기획서 반영: Rate Limit 방어용 0.7초 딜레이
        if (!bulkAbortController.signal.aborted) await new Promise(r => setTimeout(r, 700));
    }
    
    progressEl.remove(); 
    
    // 🚨 번역 완료 또는 중단 시 다시 원래의 번개 아이콘으로 원상복구
    ...
    if (bulkAbortController.signal.aborted) {
        ...
        catNotify(`${getCompletionEmoji()} 전체 번역 완료! ${completed}개 번역${skipped > 0 ? ', ' + skipped + '개 스킵' : ''}`, "success");
    }
